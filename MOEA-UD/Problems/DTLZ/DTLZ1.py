...
def evaluate(P, m):
    """Evaluates a population for the DTLZ1 test problem"""
    N, n = np.shape(P)
    g = 100*(n-m+1+np.sum((P[:,m-1:]-0.5)**2-np.cos(20*np.pi*(P[:,m-1:]-0.5)), axis=1, keepdims=True))
    return 0.5*np.tile(1+g, (1, m))*np.fliplr(np.cumprod(np.hstack((np.ones((N, 1)), P[:,:m-1])), axis=1))*np.hstack((np.ones((N, 1)), 1-P[:,m-2::-1]))

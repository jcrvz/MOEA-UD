...
def evaluate(P, m):
    """Evaluates a population for the WFG1 test problem"""
    N, n = np.shape(P)
    k = m-1
    D = 1
    A = np.ones(m-1)
...
def b_flat(y, A, B, C):
    ...
    out = roundHalfAwayFromZero(out*1e4)/1e4
    return out
...
def roundHalfAwayFromZero(x):
    """Rounds each element of x to the nearest integer. In case of a tie, rounds away from zero to the nearest integer with larger magnitude"""

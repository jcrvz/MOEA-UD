...
def evaluate(P, m):
    """Evaluates a population for the WFG9 test problem"""
    N, n = np.shape(P)
    k = m-1
    D = 1
    A = np.ones(m-1)
    ...
    y_prime = (np.fliplr(np.cumsum(np.fliplr(z01), axis=1))-z01)/np.tile(np.hstack((np.arange(n-1, 0, -1), 1e-6)), (N, 1))
    t1[:,:-1] = z01[:,:-1]**(0.02+(50-0.02)*(0.98/49.98-(1-2*y_prime[:,:-1])*np.abs(np.floor(0.5-y_prime[:,:-1])+0.98/49.98)))
    # --------
    t1[:,n-1] = z01[:,n-1]
    ...
    for i in range(k, n-1):
        for j in range(i+1, n):
            s += np.abs(t2[:,i]-t2[:,j])
    t3[:,m-1] = (np.sum(t2[:,k:], axis=1)+s*2)/np.ceil((n-k)/2)/(1+2*(n-k)-2*np.ceil((n-k)/2))
    # --------
